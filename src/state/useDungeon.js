...
export default function useDungeon(initialRunId = null) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setError(e);
      try { emit("toast", { message: "Failed to save dungeon run.", type: "error" }); } catch (_) {}
    }
  }, [run]);
  ...
  function loadRun(id) {
    ...
    try {
      ...
      setRun(s || null);
      setError(null);
      try { emit("toast", { message: `Loaded run ${id}.`, type: "info" }); } catch (_) {}
    } catch (e) {
      console.error("[useDungeon] loadRun failed", e);
      setError(e);
      try { emit("toast", { message: "Failed to load run.", type: "error" }); } catch (_) {}
    } finally {
      setLoading(false);
    }
  }
  ...
  function removeRun(id) {
    try {
      const ok = clearDungeon(id);
      if (ok) {
        try { emit("toast", { message: "Saved run removed.", type: "info" }); } catch (_) {}
      }
      return ok;
    } catch (e) {
      console.error("[useDungeon] removeRun failed", e);
      try { emit("toast", { message: "Failed to remove run.", type: "error" }); } catch (_) {}
      return false;
    }
  }
  ...
  if (def) {
    if (!meta.enemyList || !Array.isArray(meta.enemyList) || meta.enemyList.length === 0) {
      const fullPool = flattenEnemyPool(def.enemyPool || {});

      // 🔥 Remove boss from normal room pool
      meta.enemyList = fullPool.filter(id => id !== bossId);
    }

    if (typeof meta.minEnemiesPerRoom === "undefined")
      meta.minEnemiesPerRoom = def.minEnemiesPerRoom ?? 1;

    if (typeof meta.maxEnemiesPerRoom === "undefined")
      meta.maxEnemiesPerRoom = def.maxEnemiesPerRoom ?? 3;

    if (!meta.boss && def.boss)
      meta.boss = def.boss;
  ...
  if (Array.isArray(opts.enemyList) && opts.enemyList.length > 0) {
    meta.enemyList = opts.enemyList.slice().filter(id => id !== bossId);
  }
  ...
  try {
    ...
    console.log(
      "[useDungeon] startNewRun created",
      snapshot.id,
      "key:",
      snapshot.dungeonKey,
      "seed:",
    ...
    try {
      emit("toast", { message: `Started run: ${snapshot.dungeonKey}`, type: "success" });
    } catch (_) {}
  } catch (e) {
    console.error("[useDungeon] startNewRun save failed", e);
    try {
      emit("toast", { message: "Failed to start run (save error).", type: "error" });
    } catch (_) {}
  }
  ...
  function exitRun({ persistGlobalHP = false } = {}) {
    ...
    setRun(null);
    try { emit("toast", { message: "Exited run.", type: "info" }); } catch (_) {}
  }
  ...
  function canMove(dir) {
    ...
    switch (dir) {
      ...
      case "right": return inBounds(x, y + 1);
  ...
  function finishDungeon(next) {
    ...
    try {
      ...
      try { emit("toast", { message: `Dungeon cleared: ${updated.dungeonKey}`, type: "success" }); } catch (_) {}
    } catch (e) {
      console.error("[useDungeon] finishDungeon save failed", e);
      try { emit("toast", { message: "Failed to persist finished run.", type: "error" }); } catch (_) {}
    }

    // Emit a dungeon_clear event immediately so quest systems and listeners react to the clear.
    // Provide both dungeonId and dungeonKey for compatibility with various consumers.
    try {
      emit("dungeon_clear", {
        dungeonId: updated.dungeonKey,
        dungeonKey: updated.dungeonKey,
        runId: updated.id,
        rewards: updated.clearRewards,
      });
      console.log("[useDungeon] emitted dungeon_clear", updated.dungeonKey, updated.id);
    } catch (e) {
      console.error("[useDungeon] emitting dungeon_clear failed", e);
    }
  ...
  function claimDungeonRewards() {
    ...
    if (run.rewardsClaimed) {
      ...
      try { emit("toast", { message: "Rewards already claimed.", type: "info" }); } catch (_) {}
      return run;
    }
    ...
    try {
      ...
      try {
        ...
        try { emit("toast", { message: "Dungeon rewards granted.", type: "success" }); } catch (_) {}
      } catch (e) {
        ...
        try { emit("toast", { message: "Dungeon rewards applied.", type: "success" }); } catch (_) {}
      }
      ...
      try { emit("toast", { message: "Failed to claim rewards.", type: "error" }); } catch (_) {}
      return run;
    }
  }
  ...
  function enterRoom(x, y) {
    ...
    if (!enemies || enemies.length === 0) {
      ...
      try {
        ...
        try { emit("toast", { message: `Encounter: ${Array.isArray(enemies) ? enemies.join(", ") : "unknown"}`, type: "info" }); } catch (_) {}
      } catch (e) { console.error("[useDungeon] enterRoom save failed", e); try { emit("toast", { message: "Failed to generate encounter.", type: "error" }); } catch (_) {} }
  ...
  const handleBattleFinish = useCallback((battleState = {}, tileIndex = null) => {
    ...
    if (Number.isFinite(Number(tileIndex)) && tileIndex >= 0 && tileIndex < next.tiles.length) {
      ...
      if (battleState.result === "win") {
        ...
        try { emit("toast", { message: "You won the fight.", type: "success" }); } catch (_) {}
      } else {
        // On loss or draw, clear inProgress so room can be retried or regenerated as desired
        t.inProgress = false;
        try { emit("toast", { message: "You were defeated.", type: "error" }); } catch (_) {}
      }
    ...
    if (def && def.boss) {
      ...
      if (!next.bossTriggered && (next.visitedCount || 0) >= nonBossCount && !next.finished) {
        ...
        try { emit("toast", { message: "Boss unlocked!", type: "info" }); } catch (_) {}
      }
      ...
      if (Number.isFinite(Number(tileIndex)) && tileIndex === centerIdx && battleState.result === "win") {
        ...
        try {
          ...
          try { emit("toast", { message: "Boss defeated — dungeon cleared!", type: "success" }); } catch (_) {}
        } catch (e) {
          console.error("[useDungeon] handleBattleFinish save failed after finish", e);
          try { emit("toast", { message: "Dungeon finished but failed to persist.", type: "error" }); } catch (_) {}
        }
        return;
      }
      ...
      if (next.bossTriggered && battleState.result === "win" && (tileIndex === null || tileIndex === undefined)) {
        ...
        try {
          ...
          try { emit("toast", { message: "Dungeon cleared!", type: "success" }); } catch (_) {}
        } catch (e) {
          console.error("[useDungeon] handleBattleFinish save failed after finish (no tileIndex)", e);
          try { emit("toast", { message: "Dungeon cleared but failed to persist.", type: "error" }); } catch (_) {}
        }
        return;
      }
    } else {
    ...
    try {
      ...
      try { emit("toast", { message: "Failed to save run after battle.", type: "error" }); } catch (_) {}
    }
  }, [run]);
  ...
  function regenerateRoom(x, y, opts = {}) {
    ...
    try {
      saveActiveDungeonSnapshot(next);
      try { emit("toast", { message: `Room regenerated: ${Array.isArray(enemies) ? enemies.join(", ") : "unknown"}`, type: "info" }); } catch (_) {}
    } catch (e) { console.error("[useDungeon] regenerateRoom save failed", e); try { emit("toast", { message: "Failed to regenerate room.", type: "error" }); } catch (_) {} }
  ...
  function startBossFight(opts = {}) {
    ...
    if (!run.bossTriggered) {
      ...
      try { emit("toast", { message: "Boss not unlocked yet.", type: "error" }); } catch (_) {}
      return run;
    }
    const def = findDungeonDef(run.dungeonKey);
    ...
    try {
      saveActiveDungeonSnapshot(next);
      try { emit("toast", { message: "Boss fight started.", type: "info" }); } catch (_) {}
    } catch (e) {
      console.error("[useDungeon] startBossFight save failed", e);
      try { emit("toast", { message: "Failed to start boss fight.", type: "error" }); } catch (_) {}
    }
    return next;
  }
